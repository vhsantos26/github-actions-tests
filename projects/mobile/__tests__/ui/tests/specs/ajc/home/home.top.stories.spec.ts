...
describe('Home screen Top Stories section AJC @homepage @ajc', () => {
  before(async () => {
    await SetupScreen.header.waitForDisplayed();

    const enableChannel = ['ajc'];
    var disableChannel: string[] = ['aje', 'ajm', 'ajb', 'aja', 'ajd'];
  ...
  it('each Post Title should have more than 8 characters', async () => {
    // scrolling to the top before starting the test
    await Gestures.swipeUntilElementPresent(
      await HomeScreen.topStoriesLabel,
      10,
    ...
    for (let i = 0; i < postTitleArray.length; i++) {
      ...
      // setting characters limit to 8 for AJC as it has less characters in post titles
      await expect(postTitleArray[i].length).toBeGreaterThanOrEqual(8);
    }
  });
